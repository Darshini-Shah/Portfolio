...
const skillCategories = [
  {
    ...
    icon: (
      <svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
    )
  },
  {
    title: 'Core CS',
  ...
  },
  {
    ...
    skills: ['Node.js', 'FastAPI', 'REST APIs', 'WebSockets', 'System Design Basics'],
    icon: (
      <svg className="w-6 h-6 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-7 8h.01" /></svg>
    )
  },
  {
    title: 'Databases',
  ...
  {
    title: 'AI & LLMs',
    skills: ['RAG', 'LangChain', 'LangGraph', 'Prompt Engineering', 'Text Processing'],
    icon: (
  ...
  },
  {
    ...
    skills: ['Supervised Learning', 'Deep Learning (CNNs, RNNs)'],
    icon: (
  ...
  },
  {
    ...
    skills: ['Docker', 'Git', 'Scikit-learn'],
    icon: (
  ...
  {
    title: 'Data Handling',
    skills: ['Data Preprocessing', 'PDF Parsing', 'Feature Engineering'],
    icon: (
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 text-center">
          ...
          <p className="text-gray-500 max-w-xl mx-auto leading-relaxed text-sm">
            My technical toolkit spans across language proficiency, core computer science, and cutting-edge AI/ML engineering.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {skillCategories.map((category, idx) => (
            <div key={idx} className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-white hover:shadow-md transition-all group">
              <div className="flex items-center gap-3 mb-5">
                <div className="bg-gray-50 w-10 h-10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                  {/* Scaling the icon slightly too */}
                  <div className="scale-90">{category.icon}</div>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, i) => (
                  <span 
                    key={i} 
                    className="px-3 py-1 bg-gray-50 text-gray-500 text-xs font-semibold rounded-lg border border-gray-100 hover:bg-teal-50 hover:text-teal-600 hover:border-teal-100 transition-colors cursor-default"
                  >
                    {skill}
                  </span>
                ))}
